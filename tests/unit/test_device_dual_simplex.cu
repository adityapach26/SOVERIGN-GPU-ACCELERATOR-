...
__global__ void run_gpu_dual_simplex(
    ...
    gpu::dual_simplex_kernel(model, lu, ws, obj_sign, max_iter, status_out);
}

TEST_CASE("Pre-Phase 13.1 Foundation - Single Node Dual Simplex", "[cuda][dual]") {
    ...
    std::vector<Float> host_x = {0.0, 0.0, -2.0, -3.0};
    ...
    // Expected optimal solution: x1 = 1, x2 = 1, s1 = 0, s2 = 0
    ...
    run_gpu_dual_simplex<<<1, 32>>>(d_model, d_lu, ws, 1.0, 100, d_status);
    ...
    // Compare GPU solution to CPU solution
    for (Index i = 0; i < 4; ++i) {
