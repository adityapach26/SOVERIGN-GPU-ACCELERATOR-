...
__global__ void wmma_compute_residual(
    ...
    __shared__ half X_tile[WMMA_K * WMMA_N];
    ...
    for (int k_start = 0; k_start < cols; k_start += WMMA_K) {
        ...
        if (lane_id < WMMA_M * WMMA_K) {
            A_tile[lane_id] = __float2half(0.0f);
        }
        if (lane_id < WMMA_K * WMMA_N) {
            X_tile[lane_id] = __float2half(0.0f);
        }
        __syncwarp();
            ...
            if (col < cols) {
                ...
                for (Index p = start; p < end; ++p) {
                    ...
                    if (row >= row_start && row < row_start + WMMA_M) {
                        A_tile[(row - row_start) * WMMA_K + lane_id] = __float2half(static_cast<float>(A_values[p]));
                    }
                }
                // X is a vector, treated as a matrix with non-zeros only in the 0th column
                X_tile[lane_id * WMMA_N + 0] = __float2half(static_cast<float>(x[col]));
            }
        }
        __syncwarp();
    ...
    __shared__ float R_tile[WMMA_M * WMMA_N];
    wmma::store_matrix_sync(R_tile, acc_frag, WMMA_N, wmma::mem_row_major);
    __syncwarp();

    if (lane_id < WMMA_M) {
        int row = row_start + lane_id;
        if (row < rows) {
            float ax = R_tile[lane_id * WMMA_N + 0];
...
__global__ void wmma_compute_direction(
    ...
    __shared__ half AT_tile[WMMA_M * WMMA_K];
    __shared__ half R_tile[WMMA_K * WMMA_N];

    for (int j_start = 0; j_start < rows; j_start += WMMA_K) {
        if (lane_id < WMMA_M * WMMA_K) {
            AT_tile[lane_id] = __float2half(0.0f);
        }
        if (lane_id < WMMA_K * WMMA_N) {
            R_tile[lane_id] = __float2half(0.0f);
        }
        __syncwarp();
            ...
            if (col < cols) {
                ...
                for (Index p = start; p < end; ++p) {
                    ...
                    if (row >= j_start && row < j_start + WMMA_K) {
                        AT_tile[lane_id * WMMA_K + (row - j_start)] = __float2half(static_cast<float>(A_values[p]));
                    }
                }
            }
        }
        ...
        if (lane_id < WMMA_K) {
            ...
            if (row < rows) {
                R_tile[lane_id * WMMA_N + 0] = __float2half(static_cast<float>(r[row]));
            }
        }
        __syncwarp();
        ...
        wmma::load_matrix_sync(a_frag, AT_tile, WMMA_K);
        wmma::load_matrix_sync(r_frag, R_tile, WMMA_N);

        wmma::mma_sync(acc_frag, a_frag, r_frag, acc_frag);
        __syncwarp();
    }

    __shared__ float D_tile[WMMA_M * WMMA_N];
    wmma::store_matrix_sync(D_tile, acc_frag, WMMA_N, wmma::mem_row_major);
    __syncwarp();

    if (lane_id < WMMA_M) {
        int col = col_start + lane_id;
        if (col < cols) {
            float at_r = D_tile[lane_id * WMMA_N + 0];
...
std::vector<Float> compute_fp_projection_direction(
    ...
    cudaMemcpy(d_x_tilde, x_tilde.data(), model.cols * sizeof(Float), cudaMemcpyHostToDevice);
    ...
    if (err != cudaSuccess) {
        throw std::runtime_error("wmma_compute_residual kernel failed");
    }
    ...
    if (err != cudaSuccess) {
        throw std::runtime_error("wmma_compute_direction kernel failed");
    }

    cudaDeviceSynchronize();

    std::vector<Float> direction(model.cols);
    cudaMemcpy(direction.data(), d_d, model.cols * sizeof(Float), cudaMemcpyDeviceToHost);
